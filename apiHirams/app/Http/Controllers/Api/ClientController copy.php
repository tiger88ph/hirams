...
class ClientController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        try {
            ...
            // Auto-create matching journal account under "Collection"
            $collectionAccount = JournalAccount::where('strAccountName', 'Collection')
            ...
            if ($collectionAccount) {
                ...
                $exists = JournalAccount::where('nParentAccountId', $collectionAccount->nJournalAccountId)
                    ->where('strAccountName', $accountName)
                    ->exists();

                if (!$exists) {
                    JournalAccount::create([
                        'strAccountName'   => $accountName,
                        'nParentAccountId' => $collectionAccount->nJournalAccountId,
                    ]);
                }
            }
    ...
    public function update(Request $request, int $id): JsonResponse
    {
        try {
            ...
            $oldNickname = $client->strClientNickName ?: $client->strClientName;

            $client->update($validated);
            $client->refresh();

            $newNickname = $client->strClientNickName ?: $client->strClientName;
            ...
            if ($oldNickname !== $newNickname) {
                ...
                if ($collectionAccount) {
                    $oldAccountName = "Receivables from {$oldNickname}";
                    $newAccountName = "Receivables from {$newNickname}";

                    $journalAccount = JournalAccount::where('nParentAccountId', $collectionAccount->nJournalAccountId)
                        ->where('strAccountName', $oldAccountName)
                        ->first();

                    if ($journalAccount) {
                        // avoid clobbering an existing account with the target name
                        $clash = JournalAccount::where('nParentAccountId', $collectionAccount->nJournalAccountId)
                            ->where('strAccountName', $newAccountName)
                            ->where('nJournalAccountId', '!=', $journalAccount->nJournalAccountId)
                            ->exists();

                        if (!$clash) {
                            $journalAccount->update(['strAccountName' => $newAccountName]);
