...
class PurchaseOrderController extends Controller
{
            ...
            if ($validated['cStatus'] === $cancelCartKey) {
                ...
                foreach ($purchaseOrder->purchaseOrderOptions as $poOption) {
                    PurchaseItemHistory::create([
                        'nPurchaseOrder_ItemId' => $poOption->nPurchaseOrder_ItemId,
                        'nStatus'                 => $cancelPoKey,
    ...
    public function syncPurchaseOrderStatus(Request $request): JsonResponse
    {
        try {
            $validated = $request->validate([
                'nPurchaseOrderId'  => 'required|integer|exists:tblpurchaseorders,nPurchaseOrderId',
                'nPurchaseItemId' => 'required|integer',   // ← add
                'nUserId'           => 'nullable|integer',
            ...
            $purchaseOrder = PurchaseOrder::with([
                'purchaseOrderOptions' => function ($q) use ($validated) {
                    ...
                    $q->where('nPurchaseOrder_ItemId', function ($sub) use ($validated) {
                        $sub->select('nPurchaseOrder_ItemId')
            ...
            foreach ($options as $poOption) {
                ...
                $receivedQty = (int) Inventory::where('nPurchaseItemId', $po->nPurchaseItemId)
                    ->where('nQuantity', '>', 0)
                    ->whereIn('cStatus', ['A', 'P'])  // ✅ Include A + P
                    ->sum('nQuantity');

                $deliveredQty = (int) abs(Inventory::where('nPurchaseItemId', $po->nPurchaseItemId)
                    ->where('nQuantity', '<', 0)
                    ->whereIn('cStatus', ['A', 'P'])  // ✅ Include A + P
                    ->sum('nQuantity'));
