...
function ContactModal({ open, handleClose, supplier, onUpdate }) {
  const [contactList, setContactList] = useState([]);
  const [selectedIndex, setSelectedIndex] = useState(null);
  const [formData, setFormData] = useState({ strName: "", strNumber: "", strPosition: "", strDepartment: "" });
  ...
  useEffect(() => {
    if (supplier?.contacts) {
      setContactList(Array.isArray(supplier.contacts) ? supplier.contacts : [supplier.contacts]);
    } else {
      setContactList([]);
    }
  }, [supplier]);
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    setLoadingMessage(selectedIndex !== null ? `Updating ${name}...` : `Adding ${name}...`);

    try {
      const payload = { ...formData, nSupplierId: supplier.nSupplierId };

      if (selectedIndex !== null) {
        await api.put(`supplier-contacts/${contactList[selectedIndex].nSupplierContactId}`, payload);
      } else {
        await api.post("supplier-contacts", payload);
      }

      const supplierResp = await api.get("suppliers");
      const updatedSupplier = supplierResp.suppliers.find(s => s.nSupplierId === supplier.nSupplierId);
      ...
      showToast(selectedIndex !== null ? `Contact "${name}" updated successfully!` : `Contact "${name}" added successfully!`);
    } catch (error) {
      console.error(error);
  ...
  return (
    <ModalContainer open={open} handleClose={() => { setIsEditing(false); setDeleteIndex(null); handleClose(); }} title="Supplier Contacts" onSave={handleSave} loading={loading} showFooter={deleteIndex === null && isEditing}>
      {toast.open && <Alert severity={toast.severity} variant="filled" sx={{ mb: 2, color: "#fff" }}>{toast.message}</Alert>}
      {loading && (
      ...
      {deleteIndex !== null ? (
        <Box sx={{ minHeight: 200 }}>
          <Box sx={{ mb: 2, display: "flex", alignItems: "center", cursor: "pointer" }} onClick={() => setDeleteIndex(null)}>
            ...
            <Typography variant="caption" sx={{ ml: 0.5, color: "#1976d2" }}>Back to contacts</Typography>
          </Box>
          <Typography sx={{ mb: 2 }}>Delete Verification for <strong>{contactList[deleteIndex]?.strName}</strong></Typography>
          <TextField label="Enter first letter of name" value={deleteLetter} onChange={(e) => setDeleteLetter(e.target.value)} error={!!deleteError} helperText={deleteError} fullWidth />
          <Box sx={{ mt: 2 }}>
            ...
            <Grid container spacing={2}>
              {contactList.map((c, index) => (
                <Grid item xs={12} key={index}>
                  <Box sx={{ position: "relative", display: "flex", justifyContent: "space-between", alignItems: "center", bgcolor: "#e3f2fd", borderRadius: 2, p: 2, cursor: "pointer", "&:hover": { bgcolor: "#d2e3fc" } }} onClick={() => handleEditContact(index)}>
                    <IconButton size="small" onClick={e => { e.stopPropagation(); handleDeleteContact(index); }} sx={{ position: "absolute", top: 4, right: 4, bgcolor: "#fff", width: 24, height: 24, "&:hover": { bgcolor: "#f0f0f0" } }}>
                    ...
                    <ContactPhoneIcon sx={{ fontSize: 70, color: "#1976d2", opacity: 0.9 }} />
                  </Box>
                </Grid>
              ))}
            </Grid>
          ) : (
            <Typography variant="body2" align="center" sx={{ color: "gray", fontStyle: "italic", py: 3, bgcolor: "#e3f2fd", borderRadius: 2 }}>No contact registered.</Typography>
          )}
        ...
        <Grid container spacing={1.5}>
          ...
          <Grid item xs={12}><TextField label="Contact Number" name="strNumber" fullWidth size="small" value={formData.strNumber} onChange={handleChange} error={!!errors.strNumber} helperText={errors.strNumber || ""} /></Grid>
