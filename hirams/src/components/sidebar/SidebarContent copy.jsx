...
const TransactionSubItems = ({ onItemClick, selectedCode, onSelect }) => {
  ...
  // ── ONLY statusMap branches Finance separately (uses financestatus) ──
...
const cacheKey = useMemo(
  ...
  [userId, isManagement, isFinanceOfficer, isProcurement, isAOTL],
);
...
const fetchSilent = useCallback(async () => {
  ...
  try {
    ...
    if (isManagement) {
      ...
      const res = await api.get(`transaction/finance?nUserId=${userId}`);
      list = (res.transactions || res.data || []).filter(Boolean);
...
const TransactionNav = ({ collapsed, forceExpanded, onItemClick }) => {
  ...
  const sessionKey = isManagement
    ...
    : isProcurement
      ...
      : isFinanceOfficer
        ? "selectedStatusCode" // ← shares Management's session key, since it's folded in
