...
const ArchiveNavSection = ({ collapsed, forceExpanded, onItemClick }) => {
  ...
  const { archiveStatus, userTypes, loading: mappingLoading } = useMapping();
  const { isManagement, isProcurement, isAccountOfficer, isAOTL, isProcurementTL } = getUserRoles(userTypes);
  ...
  const firstCode = Object.keys(safeArchiveStatus)[0] ?? "";
  ...
  // ── Fetch archived transactions for counting ──────────────────────────
  ...
  const handleParentClick = useCallback(() => {
    const first = Object.keys(safeArchiveStatus)[0];
    if (first) handleSelect(first);
  }, [safeArchiveStatus, handleSelect]);
  ...
  return (
    <div className="flex flex-col w-full mb-1.5">
      ...
      >
        {Object.entries(safeArchiveStatus).map(([code, label]) => (
          <SidebarSubmenu
            key={code}
            label={label}
