...
// Guards against the "undefined === undefined" false-positive: if the AO id
// itself is missing, we never treat a missing field on the option as a match.
...
const CartNavSection = ({ collapsed, forceExpanded, onItemClick }) => {
  ...
  const {
    cartStatus,
    userTypes: cartUserTypes,
    loading: mappingLoading,
  } = useMapping();
  const safeCartStatus = cartStatus || {};
  const firstCode = Object.keys(safeCartStatus)[0] ?? "";
  ...
  // Use the same storage helper usePurchaseCart uses — raw localStorage.getItem("user")
  // parsing was returning an object without nUserId on it, leaving currentUserId undefined.
  ...
  const handleParentClick = useCallback(() => {
    const first = Object.keys(safeCartStatus)[0];
    if (first) handleSelect(first);
  }, [safeCartStatus, handleSelect]);
  ...
  return (
    <div className="flex flex-col w-full mb-1.5">
      ...
      >
        {Object.entries(safeCartStatus).map(([code, label]) => {
