...
// Always recalculate EWT whenever supplier, quantity, or unit price changes
useEffect(() => {
  const supplierId = Number(formData.nSupplierId);
  const quantity = Number(formData.quantity);
  const unitPrice = Number(formData.unitPrice);

  // Don't recalculate if values haven't meaningfully changed
  ...
  Number(formData.nSupplierId),  // ← compare as numbers
  Number(formData.quantity),     // ← so "140" and "140.00" are treated the same
  Number(formData.unitPrice),    // ← no spurious recalculation on reformat
  cItemType,
]);
