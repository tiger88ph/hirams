...
export default function usePurchaseCartUpdate() {
  ...
  const handleBack = useCallback(() => navigate("/item-purchasing"), [navigate]);
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      // ✅ 2. Spinner runs via Swal — page content NEVER changes
      await withSpinner("Purchase Order", async () => {
        await PurchaseOrderAPI.proceedToPayment({
          nPurchaseOrderId: po?.nPurchaseOrderId,
      ...
      // ✅ 3. Notify parent to refresh data — NO skeleton flash
      window.dispatchEvent(
        new CustomEvent("cart_status_updated", {
          detail: {
            purchaseOrderId: po?.nPurchaseOrderId,
      ...
      // ✅ 4. Success message — page content stays visible
      await showSwal(
        "SUCCESS",
        {},
      ...
      await fetchPO();
      // ❌ NO navigate("/cart") — PAGE NEVER EXITS
    } catch (err) {
