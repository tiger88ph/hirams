...
export default function useForJev() {
          ...
          batch.map(async (v) => {
            try {
              const res = await api.get(
                `jev/by-link/${encodeURIComponent(v.strNumber)}`,
              );
              ...
              const balanced =
                hasJev &&
                Math.abs(
                  Number(totals.dTotalDebit || 0) -
                    Number(totals.dTotalCredit || 0),
                ) < 0.01;
  ...
  const fetchOptionStatuses = useCallback(async (voucherList) => {
    ...
    try {
      ...
      console.error("fetchOptionStatuses:", err);
    }
  }, []);

  // ── ✅ MAIN FETCH — Updated filter logic ───────────────────────────────
  const fetchVouchers = useCallback(
    async ({ silent = false, bustStorage = false } = {}) => {
      ...
      try {
        ...
        const closedOnly = data.filter(
          (v) => String(v.cStatus) === String(voucherClosedKey),
        );
